# 🏮 Lantern archive — append-only

This file is the append-only archive for [LANTERN.md](LANTERN.md)'s log. Rotated
in per SPEED AUDIT v1 item P4 (ratified 2026-07-16): "NOW + last N runs in
LANTERN.md, older entries to an append-only archive file; history archived,
never deleted." Entries below moved here verbatim, byte-identical, in the same
order they held in LANTERN.md — oldest at the bottom, newest at the top of this
file (same convention as the live log). Entries in this file are never edited
or deleted; only ever appended to (older) below the oldest surviving line, as
future rotations move more history out of LANTERN.md.

First rotation (run #12, 2026-07-16): run #9 and everything older moved here
verbatim; NOW + run #10 onward stays in LANTERN.md.
